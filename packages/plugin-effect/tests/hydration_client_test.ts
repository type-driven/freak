/**
 * Client-side hydration tests for the atom hydration pipeline.
 *
 * Tests:
 * 1. AtomRegistry.setSerializable pre-seeds value for a serializable atom
 * 2. Serializable atom get() returns pre-seeded value over default
 * 3. Multiple pre-seeded atoms resolve correctly
 * 4. Singleton identity test via island.ts import (initAtomHydration uses module registry)
 ...
 * 9. initAtomHydration() is a no-op when called with empty string
 */
...
// --- Category 2: island.ts singleton identity ---
...
Deno.test("Singleton identity: initAtomHydration uses module-level registry", () => {
  ...
  //
  // The singleton guarantee is verifiable by inspection: both initAtomHydration()
  // and the auto-init block operate on the same `const registry` at module scope.
  // The module-level auto-init runs at import time (tested implicitly by module load).
...
// --- Category 4: Full round-trip test ---
