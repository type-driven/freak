...
{
  "imports": {
    "@freak/core": "jsr:@freak/core@^0.1.0",
...
(e.g. calling `@freak/core/internal` functions directly).
...
`useAtomSet` hooks from `@freak/core/effect/island`.
...
```tsx islands/A.tsx
import { useAtomValue } from "@freak/core/effect/island";
...
```tsx islands/B.tsx
import { useAtomSet } from "@freak/core/effect/island";
import { countAtom } from "@/atoms.ts";
...
- [ ] Update `deno.json` — add `@freak/core`, `effect`, `@effect/platform`
